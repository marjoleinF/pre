...
More complex prediction ensembles can be obtained using the ```gpe()``` function. The abbreviation gpe stands for generalized prediction ensembles, which may also include hinge functions of the predictor variables as described in [@Friedman91], in addition to rules and/or linear terms. Addition of such hinge functions may further improve predictive accuracy. See the following example:
