...
\begin{enumerate}
\item The package is completely **R** based, allowing users better access to the results and more control over the parameters used for generating the prediction rule ensemble.
\item The unbiased tree induction algorithms of [@Hothorn06] is used for deriving prediction rules, by default. Alternatively, the (g)lmtree algorithm of [@Zeileis08] can be employed, or the classification and regression tree (CART) algorithm of [@Breiman84].
\item The package supports a wider range of response variable types.
\item The initial ensemble may be generated as a bagged, boosted and/or random forest ensemble.
\item Hinge functions of predictor variables may be included as baselearners, as in the multivariate adaptive regression splines (MARS) approach of [@Friedman91], using function `gpe()`.
\item Tools for explaining individual predictions are provided.  
\end{enumerate}
...
To obtain an optimal set of model-fitting parameters, function `train()` from package **`caret`** @Kuhn08 can be employed. Package **`pre`** supports this through the `caret_pre_model` object (see also `?caret_pre_model`). Note that it's best to specify the `x` and `y` arguments when using function `train()` to train the parameters of `pre()`; the use of the `formula` and `data` arguments may lead to unexpected results.
...
Users can create a tuning grid manually, but it is probably easier to use the `caret_pre_model$grid` function, e.g.:
...
Next, we apply function `train()`. Note that, in order to reduce computation time, I have specified the number of trees to be 50, but in real applications it should be left to the default value (i.e., not specified), unless you also want to tune the `ntrees` parameter:
