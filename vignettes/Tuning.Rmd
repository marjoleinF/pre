...
Function `pre()` has a substantial number of model-fitting parameters, which may be tuned so as to optimize predictive accuracy and/or interpretability (sparsity) of the final model. For many of the parameters, default settings will likely perform well. Here, we discuss the effects of several of the parameters on predictive accuracy and model complexity. We will not explain what every argument does; readers are referred to the documentation of function `pre()` for that, or readers can consult Fokkema (2020). We then illustrate how to optimize predictive accuracy for a binary classification problem.
...
save(pre_tune, file = "pre_tune.Rda")
save(pre_tune2, file = "pre_tune2.Rda")
```
