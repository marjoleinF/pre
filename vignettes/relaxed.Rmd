---
title: "More sparse, adaptive or relaxed: Fitting rule ensembles with relaxed and/or adaptive lasso"
...
A beneficial property of the lasso penalty is that it shrinks coefficients to zero. A less beneficial property is that, in the process, the lasso tends to overshrink the large coefficients too far to zero. Authors have argued that the lasso should "be considered as a *variable screener* rather than a *model selector*" (Su et al., 2017). There seems to be a trade-off between predictive accuracy and variable selection: The penalty parameter value optimal for selection will overshrink the large coefficients, making it suboptimal for prediction. The penalty parameter value optimal for prediction ("lambda.min") will undershrink the coefficients when the aim is to also perform variable selection. It yields a high true positive rate (i.e., high probability that a true predictor will be selected) at the cost of high false positive rate (i.e., high probability that a non-predictor will be selected). 

When fitting prediction rule ensembles (PREs), this may be a nuisance, because often we want to enforce sparsity. In what follows, we show how we can easily select a smaller number of terms for the final ensemble, while retaining relatively high predictive accuracy, using the relaxed lasso. Furthermore, use of the adaptive lasso may further optimize selection and prediction.
...
Function `pre` allows for adaptive lasso estimation through specification of argument `ad.alpha`. It first uses elastic net regression for computing penalty weights. 
The initial ridge estimates are also obtained using the `cv.glmnet` function. An elastic net penalty is thus applied by default for computing the weights, use of ridge regression is recommended and by specifying `ad.alpha = 0`. Lasso regression can be used by specifying `ad.alpha = 1` and OLS can be used by specifying `ad.penalty = 0`. Next, the inverse of the absolute values of the estimated coefficients are supplied as penalty factors to the `cv.glmnet` function, which will be default use `alpha = 1`, but other values for the elastic-net penalty mixing parameter can be specified. For the initial and final estimates, the same fold assignments are used in the cross validation.
...
An excellent discussion of the consistency, predictive and selection accuracy of the lasso and related methods can be found in the master's thesis of Kirkland (2014; pages 101-120). 
...
From the `Df` and `Lambda` columns, we can see that a $\lambda$ value of `r tab$Lambda[which(tab$Df == 5)[1]]` would result in a final ensemble comprising five terms. However, from the plot above, which shows the CV error ($y$-axis) against the value of penalty parameter $\lambda$ (bottom $x$-axis) and the corresponding number of selected terms (top $x$-axis), we see that this would yield substantially higher error. In part, this is due to overshrinkage, which we can mitigate using the relaxed lasso, which deshrinks the non-zero coefficients towards their unpenalized values.


### Relaxed lasso

We can employ the relaxed lasso by specifying `relax = TRUE` when fitting a rule ensemble:
...
If we specify `relax = TRUE`, the `gamma` argument (see `?cv.glmnet` for documentation on arguments `relax` and `gamma`) will by default be set to a range of five values in the interval [0, 1]. This can be overruled by specifying different values for argument `gamma` in the call to function `pre` (but which is seldom necessary). 
...
As a rule of thumb, the relaxed lasso and possibly forward-stepwise selection based on the relaxed lasso yield sparser ensembles. The relaxed adaptive lasso might perform better in terms of predictive accuracy.
...
Kirklan, L.-A. (2014). LASSO - Simultaneous shrinkage and selection via the L1 norm. Master's thesis, University of Pretoria. https://www.researchgate.net/publication/325929497_LASSO_-_Simultaneous_shrinkage_and_selection_via_the_L1_norm
